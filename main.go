...
import (
	sap_api_caller "sap-api-integrations-chat-activity-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-chat-activity-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-chat-activity-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	iD, text := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetChatActivityCollection(iD, text, accepter)
	return nil
}

func extractData(data map[string]interface{}) (iD, text string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	iD = sdc.ChatActivityCollection.ID
	text = sdc.ChatActivityCollection.ChatActivityParties.ChatActivityTextCollection.Text
	return
}
